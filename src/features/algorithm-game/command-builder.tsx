...
export function CommandBuilder(props: CommandBuilderProps) {
  ...
  return (
    <section
      className="game-panel flex min-h-[30rem] flex-col p-4 sm:p-5"
      aria-labelledby="builder-title"
    >
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="eyebrow">Seu algoritmo</p>
          <h2 id="builder-title" className="font-display text-xl font-bold">
            Monte a sequência
          </h2>
        </div>
        <span
          className={cn(
            "step-counter",
            sequence.length > challenge.maxSteps && "step-counter-danger",
          )}
        >
        ...
        {challenge.commands.map((commandId) => {
          ...
          return (
            <Button
              ...
              variant="command"
              size="command"
              onClick={() => props.onAdd(command.id)}
              disabled={isRunning}
            >
              <Icon /> {command.label}
            </Button>
        ...
        {sequence.length === 0 ? (
          <div className="flex h-full min-h-44 flex-col items-center justify-center px-5 text-center">
            ...
            <p className="mt-1 max-w-xs text-sm text-muted-foreground">
              Toque nos comandos acima para criar uma sequência.
            </p>
          </div>
        ) : (
              ...
              return (
                ...
                >
                  ...
                  <Button
                    ...
                    aria-label={`Mover ${command.label} para cima`}
                  >
                    ↑
                  </Button>
                  <Button
                    ...
                    aria-label={`Mover ${command.label} para baixo`}
                  >
                    ↓
                  </Button>
                  <Button
                    ...
                    disabled={isRunning}
                    aria-label={`Remover ${command.label}`}
                  >
                    <Trash2 />
                  </Button>
                </li>
      ...
      <div className="mt-4 grid grid-cols-[auto_auto_1fr] gap-2">
        <Button
          ...
          disabled={!sequence.length || isRunning}
          aria-label="Desfazer"
        >
          <Undo2 />
        </Button>
